...
# 真实分布 P（猫 80%, 狗 15%, 鸟 5%）
P = np.array([0.80, 0.15, 0.05])
# 模型分布 Q1（接近 P，预测猫 70%）
Q1 = np.array([0.70, 0.20, 0.10])
# 完全不同的分布 Q2（预测猫 10%）
Q2 = np.array([0.10, 0.80, 0.10])

print(f"D_KL(P||Q1) = {kl_divergence(P, Q1):.4f}  (Q1 接近 P → KL 小)")
print(f"D_KL(P||Q2) = {kl_divergence(P, Q2):.4f}  (Q2 远离 P → KL 大)")

# 不对称性验证：交换 P 和 Q
print(f"\n不对称性:")
print(f"D_KL(P||Q1) = {kl_divergence(P, Q1):.4f}")
print(f"D_KL(Q1||P) = {kl_divergence(Q1, P):.4f}")
print(f"两者不等 → KL 不是真正的'距离'，而是'信息损失'")
```

> **关键洞察**：D_KL(P‖Q) 衡量"用 Q 近似 P 需要多花多少比特"——KL 散度越小，Q 越接近 P。训练分类模型本质上就是最小化 D_KL(真实分布 ‖ 模型分布)。当 P 是 one-hot 标签时，最小化 D_KL 等价于最小化交叉熵（见 19.4 节）。
...
> **关键洞察**：PPL 作为安全护栏有它的优势（计算便宜、不需要额外模型）和局限（精心的越狱攻击可以绕过）。工业实践中通常**组合 PPL 阈值 + 内容分类器 + 规则引擎**构成多层防御。附录 E 的 Agent 速查表给出了完整的越狱检测方案对照。
