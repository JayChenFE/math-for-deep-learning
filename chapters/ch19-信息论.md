...
### 19.3~19.4　KL 散度与交叉熵 —— 两个分布的"距离"

KL 散度（Kullback-Leibler Divergence）衡量**用分布 Q 来近似真实分布 P 时，每个事件平均多用了多少比特**。

$$D_{KL}(P \parallel Q) = \sum_i p_i \cdot \log_2\frac{p_i}{q_i}$$

交叉熵 H(P, Q) 则直接给出**用 Q 编码 P 时所需的总平均比特数**：

$$H(P, Q) = -\sum_i p_i \log_2(q_i) = H(P) + D_{KL}(P \parallel Q)$$

**关键关系**：交叉熵 = 真实分布的熵 + KL 散度。H(P) 是不可压缩的"先天不确定度"，D_KL 是你用 Q 近似 P 时"多花的冤枉比特"。**训练模型的过程，就是最小化 D_KL(P||Q)——即让预测分布 Q 尽可能接近真实分布 P。**

在分类任务中，P 是 one-hot 真值标签（如 [1, 0, 0]），Q 是模型 softmax 输出（如 [0.7, 0.2, 0.1]）。H(P) = 0（one-hot 的熵为零），所以**交叉熵 H(P,Q) = D_KL(P||Q)：最小化交叉熵就是最小化 KL 散度。**

📐 **定义**：
- **KL 散度**：D_KL(P‖Q) = Σ pᵢ log(pᵢ/qᵢ) ≥ 0，等于 0 当且仅当 P=Q。非对称！
- **交叉熵**：H(P,Q) = −Σ pᵢ log(qᵢ) = H(P) + D_KL(P‖Q)。深度学习的标准损失函数。
