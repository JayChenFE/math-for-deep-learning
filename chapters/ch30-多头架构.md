...
结构：`Linear(d_model → 4*d_model) → ReLU → Linear(4*d_model → d_model)`。两个线性层中间夹一个激活函数。（实际 GPT-2/BERT 用 GeLU——ReLU 的平滑变体，`GeLU(x) ≈ x·Φ(x)`，但教学代码用 ReLU 原理完全相同。）FFN 占整个 Block 约 2/3 的参数量——**模型的知识主要存储在 FFN 的权重中，而不是 Attention 中。**

📐 **定义　FFN**：`FFN(x) = ReLU(x @ W1 + b1) @ W2 + b2`。W1 (d_model, 4*d_model), W2 (4*d_model, d_model)。每个 token 独立应用相同的 FFN。
...
class TransformerBlock:
    ...
    def forward(self, x):
        ...
        x = x + self.attention(self.layer_norm(x, self.gamma1, self.beta1))
        # Pre-Norm + FFN + Residual
        x = x + self.ffn(self.layer_norm(x, self.gamma2, self.beta2))
        return x
