...
> 💡 **Agent 视角**：在 Agent 系统中，采样策略直接决定行为风格——**工具调用（Function Calling）** 时低温度（0.1~0.3）保证 JSON 动作确定性，避免格式错误导致工具执行失败；**创意规划（CoT）** 时中等温度（0.7~0.9）鼓励生成多条推理路径，在多样性中寻找最优解；**多轮反思（Reflexion）** 时，建议对同一状态用高温度采样多条轨迹，选取验证结果最优的一条作为后续执行路径。**你的采样策略，就是 Agent 的"性格开关"。**
