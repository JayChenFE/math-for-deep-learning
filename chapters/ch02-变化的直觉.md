...
plt.plot([x1, x2], [f(x1), f(x2)], 'ro', markersize=6)  # 两个端点
...
这个 4.0 告诉我们：在这段区间上，x 每增加 1，f(x) 平均增加 4。但它没有告诉我们——在 `x=1` 这一瞬间，函数到底变化多快？
...
运行结果会让你直观感受到：h 从 1 缩小到 0.0001，割线斜率从 5.0 一路收敛到 4.0001——越来越接近真实导数 4。
...
当 `h` 小于约 `1e-12` 时，计算出的斜率不再逼近 4，反而变得剧烈跳动甚至变成 0。原因是：`f(x+h)` 和 `f(x)` 在 h 极小时几乎相等，它们相减时有效数字全部丢失——这就是**浮点舍入误差**（回顾第 1 章的 `0.1 + 0.2 ≠ 0.3`）。数值微分的黄金法则：**h 太小不行（舍入误差），太大也不行（近似粗糙），一般在 `1e-5` 到 `1e-8` 之间最合适。**

🔗 **AI 连接**：PyTorch 的 `autograd` 不需要你手动选 h——它用链式法则（第 13 章）精确计算梯度，完全避开数值微分的精度问题。这也是为什么深度学习框架必须实现自动微分，而不是简单用数值近似。
...
# 如需保存为 GIF，取消下面注释：
# ani.save('secant_to_tangent.gif', writer='pillow', fps=10, dpi=80)
```

观察动画：h 越大（初始几帧），割线几乎横跨整条抛物线，斜率远大于 4；随着 h 缩小，割线绕固定点旋转收敛，最终与绿色切线完全重合。这条切线就是"导数"的几何化身。
