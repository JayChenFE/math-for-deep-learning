...
💻 **代码　加载 GPT-2 并完成文本 Tokenization**
...
# GPT-2 默认没有 pad_token，设置为 eos_token 避免 padding 时报错
tokenizer.pad_token = tokenizer.eos_token

# Tokenize 示例文本
texts = ["这部电影太棒了!", "烂片,浪费钱", "一般般,不好不坏"]
encoded = tokenizer(texts, padding=True, truncation=True, max_length=32,
                    return_tensors='pt')

print(f"input_ids shape: {encoded['input_ids'].shape}")      # (3, 最长序列长度)
print(f"attention_mask shape: {encoded['attention_mask'].shape}")
print(f"第一个序列的 token IDs: {encoded['input_ids'][0]}")
print(f"解码验证: {tokenizer.decode(encoded['input_ids'][0])}")
...
> **关键洞察**：`padding=True` 将不等长的文本补齐到相同长度，`attention_mask` 标记哪些位置是真实 token（1）哪些是 padding（0）——这是第 20 章浮点精度的落地点：padding 位置的 logits 会被 mask 掉，不参与 loss 计算。
...
lora_attn_params = d * r + r * (3*d) + d * r + r * d  # B_A(d,r)@A_A(r,3d) + B_O(d,r)@A_O(r,d)
